...
Интерпретация: IS-IS установил соседства (UP) на уровне L2 с обоими Leaf-коммутаторами. Соседство установлено через интерфейсы Gi0/0 и Gi0/1. Состояние UP и значение Holdtime указывают на стабильность соседств.
...
Интерпретация: В базе данных IS-IS видны LSP (Link State Packets) от всех устройств, что подтверждает корректный обмен информацией о топологии. Флаги ATT/P/OL равны 0, что означает отсутствие проблем (нет перегрузки, нет ошибок).
